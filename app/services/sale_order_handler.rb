class SaleOrderHandler
	...
	def generate_order
    order = Order.create(total: @order_sum, order_qty: @qty_sum, label: @options[:label],reservation_expiry_date: @expiry_date,reserve_status: @reserve_status, paidtype: @paid_type  )
      ...
      payment_by_cash(product,new_qty,total,@order_sum,line_item_price) if @order_type == "order" && @paid_type == "Cash"
      payment_by_invoice_cash_or_card(product,new_qty,total,@order_sum,line_item_price) if @order_type == "invoice" &&
																													(@paid_type == "Invoice Cash" || @paid_type == "Invoice Card")
      create_line_item(product , new_qty, qty, total, order, line_item_price)
    end
